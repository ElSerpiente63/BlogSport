...
async def admin_login(login: Login):
    response = requests.get(f"http://{username}:{password}@127.0.0.1:5984/blogsportusers/_design/login/_view/loadlogin?key='" + login['username'] + '"')
    print(response)
    doc = response.json()
    database = couchdb.Database(f'http://{username}:{password}@127.0.0.1:5984/blogsportsession')
